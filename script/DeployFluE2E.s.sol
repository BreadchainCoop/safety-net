 ...
 *      (second member, needed for minimumMembers). Registers both the provider (A) and binding (B)
 *      TEST-key hashes and enables both domains, so the design-C two-email flow works end-to-end in
 *      the UI. TEST keys only — never production.
 */
contract DeployFluE2E is Script {
  ...
  // TEST-key values from circuits/fixtures + circuits/scripts/pubkey-hash.mjs. The provider (A) and
  // binding (B) DKIM key hashes are regenerated by the v2 fixture (circuits/scripts/gen-*).
