...
contract SafetyNet is ISafetyNet, ReentrancyGuard, OwnableUpgradeable {
  ...
  function initialize(address _owner, address[] calldata _allowedTokens) external override initializer {
    __Ownable_init_unchained(_owner);
    for (uint256 _i; _i < _allowedTokens.length; _i++) {
      allowedTokens[_allowedTokens[_i]] = true;
      emit TokenAllowed(_allowedTokens[_i], true);
    }
  }
