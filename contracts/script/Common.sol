...
contract Common is Script {
  ...
  function _deployContracts(address _admin, address[] memory _allowedTokens) internal returns (TransparentUpgradeableProxy) {
    return _deployTransparentProxy(
      address(_deploySafetyNet()), address(_deployProxyAdmin(_admin)), abi.encodeCall(SafetyNet.initialize, (_admin, _allowedTokens))
    );
  }
}
