...
abstract contract SafetyNetFuzzBase is Test {
  ...
  function setUp() public virtual {
    ...
    // proxy (initialize owner + allow token)
    address[] memory tokens = new address[](1);
    tokens[0] = address(_token);
    bytes memory initData = abi.encodeCall(SafetyNet.initialize, (_owner, tokens));
  ...
  function _deployIsolatedFund() internal returns (SafetyNet localFund, MockERC20 localToken) {
    ...
    bytes memory init = abi.encodeCall(SafetyNet.initialize, (address(this), noTokens));
