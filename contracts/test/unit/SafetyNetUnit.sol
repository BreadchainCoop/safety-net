...
contract SafetyNetUnit is Test {
  ...
  function setUp() public {
    ...
    address proxy = address(new TransparentUpgradeableProxy(impl, admin, abi.encodeCall(SafetyNet.initialize, (_owner, new address[](0)))));
    _sn = SafetyNet(proxy);
  ...
  function test_InitializeWhenAlreadyInitialized() external {
    ...
    _sn.initialize(_owner, new address[](0));
  }
  ...
  function test_InitializeWhenNotInitialized() external {
    ...
    SafetyNet fresh = SafetyNet(
      address(
        new TransparentUpgradeableProxy(
          address(new SafetyNet()), address(new ProxyAdmin(_alice)), abi.encodeCall(SafetyNet.initialize, (_alice, new address[](0)))
        )
      )
    );
    ...
    vm.expectRevert(abi.encodeWithSignature('InvalidInitialization()'));
    fresh.initialize(_alice, new address[](0));
  }
